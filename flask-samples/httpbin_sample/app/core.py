...
from flask import jsonify, Flask, request, render_template, make_response, Response, redirect


def jsonify_with_tailed_line(*args, **kwargs):
    response = jsonify(*args, **kwargs)
    if not response.data.endwith(b'\n'):
        response.data += b'\n'
    return response
...
def index():
    tracking_enable = 'TRACKING' in os.environ
    return render_template('index.html', tracking_enable=tracking_enable)
...
@app.route('/ip')
def view_ip():
    return jsonify_with_tailed_line(origin=request.headers.get('X-Forwarded-For', request.remote_addr))


@app.route('/headers')
def view_headers():
    return jsonify_with_tailed_line(get_dict('headers'))
...
def redirect_n_times(n):
    ...
    if n == 1:
        return redirect(request('view_get', _external=absolute))
...
def _redirect(kind, n, external):
    return redirect(request('{0}_redirect_n_times'.format(kind), n=n - 1, _external=external))
...
def relative_redirect_n_times(n):
    ...
    if n == 1:
        response.headers['Location'] = request('view_get')
        return response

    response.headers['Location'] = request('relative_redirect_n_times', n=n - 1)
    return response
...
def absolute_redirect_n_times(n):
    ...
    if n == 1:
        return redirect(request('view_get', _external=True))
...
def set_cookie(name, value):
    ...
    r = app.make_response(redirect(request('view_cookies')))
...
def link_page(n, offset):
    ...
    for i in range(n):
        if i == offset:
            ...
            html.append(link.format(request('link_page', n=n, offset=i), i))
...
def links(n):
    ...
    return redirect(request('link_page', n=n, offset=0))
